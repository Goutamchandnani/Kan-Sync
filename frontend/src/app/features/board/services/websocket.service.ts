...
import { BehaviorSubject, Observable, interval, Subscription, fromEvent } from 'rxjs';
...
export class WebSocketService {
  ...
  emitPublicBoardJoin(boardId: string) {
    if (this.socket) {
      this.socket.emit('joinPublicBoard', boardId);
    }
  }

  emitPublicBoardLeave(boardId: string) {
    if (this.socket) {
      this.socket.emit('leavePublicBoard', boardId);
    }
  }
